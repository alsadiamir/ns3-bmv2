...
# inject __hash__ into the thrift compiler's generated types
BmMatchParam.__hash__ = lambda self: hash((self.type, self.exact, self.lpm, self.ternary, self.valid, self.range))
BmMatchParamExact.__hash__ = lambda self: hash(self.key)
BmMatchParamLPM.__hash__ = lambda self: hash((self.key, self.prefix_length))
    ...
    def mc_dump(self):
        ...
        for mgrp in mc_json["mgrps"]:
            ...
            for L1h in mgrp["l1_handles"]:
                ...
                print("-> (ports=[{}], lags=[{}])".format(", ".join([str(p) for p in ports]), ", ".join([str(l) for l in lags])))
        print("==========")
        print("LAGS")
        if "lags" in mc_json:
