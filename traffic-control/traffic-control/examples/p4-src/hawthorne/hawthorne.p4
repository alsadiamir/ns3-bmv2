          ...
          inout standard_metadata_t standard_metadata) 
{
  ...
  register<bit<32>>(1) thr_s;
  register<bit<16>>(1) bg_cval_s;
  ...
  apply { 
    ...
    thr_s.read(thr_tmp, 0);
    bg_cval_s.read(bg_cval, 0);
          ...
          if(bucket_stamp > last_stamp) {
            ...
            if (nval > meta.stats.Xsum + (STDEV_RANGE * meta.stats.StdNX)) { // compare Nx
              if(saved_nval == 0){
                bit<16> bg_cval_tmp;
                if(bucket_idx > 0){
                  read_bucket(bg_cval_tmp, bucket_idx-1, COUNTER_TOPLEVEL);
                } else{
                  read_bucket(bg_cval_tmp, WINDOW_SIZE-1, COUNTER_TOPLEVEL);
                }
                bg_cval_s.write(0, bg_cval_tmp);
              }
            ...
            if(sent == 1){
              ...
              //   // dec_trend_s.write(0, 0);
              // }
              if(10*cval <= 11*bg_cval){
                  // tmp_spike_despec_s.write(0, 10*cval);
                  nval_despec.write(0, (bit<32>)cval);
                  nval_despec.write(1, (bit<32>)bg_cval);
                  // nval_despec.write(2, 10*meta.stats.Xsum);
                  // nval_despec.write(3, STDEV_RANGE_DESPEC * meta.stats.StdNX);
                  reset = 1;
                  reset_s.write(0, reset);
